...
namespace Componentes_ProyectoGimnasioMMA.Componentes.GestionPersonas.Listar;

public class ListarAlumno : ContentPage
{
    // Recursos
    ...
    public event Action<Alumno> AlumnoSeleccionadaEvento;
    ...
    public ListarAlumno(Escuela escuela)
    {
    ...
    private void GenerarInterfaz()
    {
        ...
        {
            _mailVSL.Children.Add(GeneracionUI.CrearCartaAlumno(alumno, CartaClickeada));
        }
                    ...
                    if (layout.Children[0] is Label nombreLabel)
                    {
                        for (int indice = 0; indice < _listaAlumnos.Count; indice++)
                        {
                            if (_listaAlumnos[indice].DNI == nombreLabel.Text)
                            {
                                ...
                                AlumnoSeleccionadaEvento?.Invoke(_alumnoElegido);
